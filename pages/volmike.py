        ...
        def window_analysis(start: int, end: int) -> dict:
            ...
            for pos in range(start, end + 1):
                ...
                if "BBW_Tight_Emoji" in intraday.columns:
                    val = intraday["BBW_Tight_Emoji"].iloc[pos]
                    if val == "🐝":
                        bishops["yellow"] += 1

                # Purple = STD Expansion ♗
                if "STD_Alert" in intraday.columns:
                    val = str(intraday["STD_Alert"].iloc[pos])
                    if val not in ("", "nan", "None"):
                        bishops["purple"] += 1

                # Green/Red = BBW Expansion ♗
                if "BBW Alert" in intraday.columns:
                    val = str(intraday["BBW Alert"].iloc[pos])
                    if val not in ("", "nan", "None"):
                        kijun = pd.to_numeric(intraday["Kijun_F"].iloc[pos] if "Kijun_F" in intraday.columns else None, errors="coerce")
                        fv    = pd.to_numeric(intraday["F_numeric"].iloc[pos], errors="coerce")
                ...
                if rv_col is not None:
                    rv = pd.to_numeric(intraday[rv_col].iloc[pos], errors="coerce")
